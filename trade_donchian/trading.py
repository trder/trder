from trder import *
#donchian
def entry_signal(exchange,symbol,param) -> dict: #入市信号
  e,s = exchange,symbol
  risk = 1.0
  don_days = param['-do1']
  if TOTAL_POS() > 0:
    return None
  pos = MARGIN() * risk / ATRP10D[e,s]
  if DONBREAK(don_days,e,s) == 1:
    strategy = {
    "sign":1.0, #信号强度
    "side":"buy", #方向：做多buy或做空sell
    "pos":pos #头寸大小：（以USD为单位）
    }
  elif DONBREAK(don_days,e,s) == -1:
    strategy = {
    "sign":1.0, #信号强度
...
def exit_signal(order,param) -> tuple: #退出信号
